...
public class PublicHoliday {
	...
	private static void addHoliday() {
		...
		if(dayEve>0) //if d-1>0 then it is a valid date
		...
		else { //if d-1=0 then it is a not valid date, month-1 and find the max date of month-1
			...
			else { //when eve and day itself is not in the same year example new year and new year eve
	...
	private static void deleteHoliday() {
		...
		while(true) {
			while(true) {
				System.out.println("Enter the date to delete in dd/MM/yyyy format (eve will be automatically deleted):");
				String date = sc.nextLine();
				String[] parts = date.split("/"); //split the string to get dd, MM and YYYY individually
				try {
					d = Integer.parseInt( parts[0] );
